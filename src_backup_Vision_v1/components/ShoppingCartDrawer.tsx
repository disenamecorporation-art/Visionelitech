...
import { EXCHANGE_RATE_VES } from "../data";
...
export default function ShoppingCartDrawer({
  ...
  const handleCheckoutWhatsApp = () => {
    ...
    messageText += `*TOTAL EN VES (Tasa ${EXCHANGE_RATE_VES}):* Bs. ${totalVES.toLocaleString(undefined, { maximumFractionDigits: 0 })} VES\n\n`;
      ...
      <div className="relative w-full max-w-md h-full bg-[#07070a] border-l border-white/10 flex flex-col justify-between shadow-2xl animate-in slide-in-from-right duration-300">
            ...
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div>
                ...
                <p className="font-sans text-[11px] text-white/40 max-w-xs mt-1">
                  ¡Añade productos de la Tienda WooCommerce o diseña tu PC personalizada en la pestaña de armado virtual!
                </p>
              </div>
            </div>
          ) : (
            cartItems.map((item) => {
              const itemVES = (item.product.priceUSD * EXCHANGE_RATE_VES * item.quantity).toLocaleString(undefined, {
        ...
        <div className="p-6 border-t border-white/5 bg-black/60 space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between items-baseline text-xs font-sans text-white/40">
              <span>SUBTOTAL VES (REFERENCIAL)</span>
              <span className="text-white/80 font-bold">
                Bs. {totalVES.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </span>
            </div>
            <span className="text-[9px] text-white/30 block text-right font-mono leading-none">
              * Tasa oficial {EXCHANGE_RATE_VES} Bs./USD.
            </span>
          </div>
